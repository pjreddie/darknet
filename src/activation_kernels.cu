...
{
    binary_gradient_array_kernel<<<cuda_gridsize(n/2), BLOCK, 0, get_darknet_stream()>>>(x, dx, n/2, size, a, y);
...
{
    binary_activate_array_kernel<<<cuda_gridsize(n/2), BLOCK, 0, get_darknet_stream()>>>(x, n/2, size, a, y);
    check_error(cudaPeekAtLastError());
}
...
{
    activate_array_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(x, n, a);
    check_error(cudaPeekAtLastError());
}

extern "C" void gradient_array_gpu(float *x, int n, ACTIVATION a, float *delta) 
{
    gradient_array_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(x, n, a, delta);
    check_error(cudaPeekAtLastError());
}
