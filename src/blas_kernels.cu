...
void scale_bias_gpu(float *output, float *biases, int batch, int n, int size)
{
    ...
    scale_bias_kernel<<<dimGrid, dimBlock, 0, get_darknet_stream()>>>(output, biases, n, size);
...
{
    backward_scale_kernel<<<n, BLOCK, 0, get_darknet_stream()>>>(x_norm, delta, batch, n, size, scale_updates);
    check_error(cudaPeekAtLastError());
}
...
void add_bias_gpu(float *output, float *biases, int batch, int n, int size)
{
    ...
    add_bias_kernel<<<cuda_gridsize(num), BLOCK, 0, get_darknet_stream()>>>(output, biases, batch, n, size);
    check_error(cudaPeekAtLastError());
}
...
{
    if(size == 1){
        backward_bias_conn_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(bias_updates, delta, batch, n);
    }else{
        backward_bias_kernel<<<n, BLOCK, 0, get_darknet_stream()>>>(bias_updates, delta, batch, n, size);
    }
    check_error(cudaPeekAtLastError());
}
...
{
    dot_kernel<<<cuda_gridsize(l.n*l.n), BLOCK, 0, get_darknet_stream()>>>(l.output_gpu, l.dot, l.batch, l.n, l.out_w * l.out_h, l.delta_gpu);
    check_error(cudaPeekAtLastError());
}
*/
...
{
    adam_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, x, m, v, B1, B2, rate, eps, t);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void normalize_delta_gpu(float *x, float *mean, float *variance, float *mean_delta, float *variance_delta, int batch, int filters, int spatial, float *delta)
{
    ...
    normalize_delta_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, x, mean, variance, mean_delta, variance_delta, batch, filters, spatial, delta);
    check_error(cudaPeekAtLastError());
}
...
{
    mean_delta_kernel<<<cuda_gridsize(filters), BLOCK, 0, get_darknet_stream()>>>(delta, variance, batch, filters, spatial, mean_delta);
    check_error(cudaPeekAtLastError());
}

extern "C" void fast_mean_delta_gpu(float *delta, float *variance, int batch, int filters, int spatial, float *mean_delta)
{
    fast_mean_delta_kernel<<<filters, BLOCK, 0, get_darknet_stream()>>>(delta, variance, batch, filters, spatial, mean_delta);
    check_error(cudaPeekAtLastError());
}
...
{
    fast_variance_delta_kernel<<<filters, BLOCK, 0, get_darknet_stream()>>>(x, delta, mean, variance, batch, filters, spatial, variance_delta);
    check_error(cudaPeekAtLastError());
}
...
{
    size_t N = batch*filters*spatial;
    normalize_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, x, mean, variance, batch, filters, spatial);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void l2normalize_gpu(float *x, float *dx, int batch, int filters, int spatial)
{
    ...
    l2norm_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, x, dx, batch, filters, spatial);
    check_error(cudaPeekAtLastError());
}
...
{
    fast_mean_kernel<<<filters, BLOCK, 0, get_darknet_stream()>>>(x, batch, filters, spatial, mean);
    check_error(cudaPeekAtLastError());
}

extern "C" void fast_variance_gpu(float *x, float *mean, int batch, int filters, int spatial, float *variance)
{
    fast_variance_kernel<<<filters, BLOCK, 0, get_darknet_stream()>>>(x, mean, batch, filters, spatial, variance);
    check_error(cudaPeekAtLastError());
}
...
{
    mean_kernel<<<cuda_gridsize(filters), BLOCK, 0, get_darknet_stream()>>>(x, batch, filters, spatial, mean);
    check_error(cudaPeekAtLastError());
}

extern "C" void variance_gpu(float *x, float *mean, int batch, int filters, int spatial, float *variance)
{
    variance_kernel<<<cuda_gridsize(filters), BLOCK, 0, get_darknet_stream()>>>(x, mean, batch, filters, spatial, variance);
    check_error(cudaPeekAtLastError());
}
...
{
    pow_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX, Y, INCY);
    check_error(cudaPeekAtLastError());
}

extern "C" void axpy_gpu_offset(int N, float ALPHA, float * X, int OFFX, int INCX, float * Y, int OFFY, int INCY)
{
    axpy_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, OFFX, INCX, Y, OFFY, INCY);
    check_error(cudaPeekAtLastError());
}
...
{
    mul_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, X, INCX, Y, INCY);
    check_error(cudaPeekAtLastError());
}

extern "C" void copy_gpu_offset(int N, float * X, int OFFX, int INCX, float * Y, int OFFY, int INCY)
{
    copy_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, X, OFFX, INCX, Y, OFFY, INCY);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void flatten_gpu(float *x, int spatial, int layers, int batch, int forward, float *out)
{
    ...
    flatten_kernel<<<cuda_gridsize(size), BLOCK, 0, get_darknet_stream()>>>(size, x, spatial, layers, batch, forward, out);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void reorg_gpu(float *x, int w, int h, int c, int batch, int stride, int forward, float *out)
{
    ...
    reorg_kernel<<<cuda_gridsize(size), BLOCK, 0, get_darknet_stream()>>>(size, x, w, h, c, batch, stride, forward, out);
    check_error(cudaPeekAtLastError());
}
...
{
    mask_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, X, mask_num, mask, val);
    check_error(cudaPeekAtLastError());
}
...
{
    scale_mask_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, X, mask_num, mask, scale);
    check_error(cudaPeekAtLastError());
}

extern "C" void const_gpu(int N, float ALPHA, float * X, int INCX)
{
    const_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void constrain_gpu(int N, float ALPHA, float * X, int INCX)
{
    constrain_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}
...
{
    add_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void scal_gpu(int N, float ALPHA, float * X, int INCX)
{
    scal_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void supp_gpu(int N, float ALPHA, float * X, int INCX)
{
    supp_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void fill_gpu(int N, float ALPHA, float * X, int INCX)
{
    fill_kernel<<<cuda_gridsize(N), BLOCK, 0, get_darknet_stream()>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void shortcut_gpu(int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float s1, float s2, float *out)
{
    ...
    shortcut_kernel<<<cuda_gridsize(size), BLOCK, 0, get_darknet_stream()>>>(size, minw, minh, minc, stride, sample, batch, w1, h1, c1, add, w2, h2, c2, s1, s2, out);
    check_error(cudaPeekAtLastError());
}
...
{
    smooth_l1_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    softmax_x_ent_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    logistic_x_ent_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    l2_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    l1_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    wgan_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}
...
{
    deinter_kernel<<<cuda_gridsize((NX+NY)*B), BLOCK, 0, get_darknet_stream()>>>(NX, X, NY, Y, B, OUT);
    check_error(cudaPeekAtLastError());
}
...
{
    inter_kernel<<<cuda_gridsize((NX+NY)*B), BLOCK, 0, get_darknet_stream()>>>(NX, X, NY, Y, B, OUT);
    check_error(cudaPeekAtLastError());
}

extern "C" void weighted_sum_gpu(float *a, float *b, float *s, int num, float *c)
{
    weighted_sum_kernel<<<cuda_gridsize(num), BLOCK, 0, get_darknet_stream()>>>(num, a, b, s, c);
    check_error(cudaPeekAtLastError());
}
...
{
    weighted_delta_kernel<<<cuda_gridsize(num), BLOCK, 0, get_darknet_stream()>>>(num, a, b, s, da, db, ds, dc);
    check_error(cudaPeekAtLastError());
}
...
{
    mult_add_into_kernel<<<cuda_gridsize(num), BLOCK, 0, get_darknet_stream()>>>(num, a, b, c);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void softmax_tree(float *input, int spatial, int batch, int stride, float temp, float *output, tree hier)
{
    ...
    softmax_tree_kernel<<<cuda_gridsize(num), BLOCK, 0, get_darknet_stream()>>>(input, spatial, batch, stride, temp, output, hier.groups, tree_groups_size, tree_groups_offset);
    check_error(cudaPeekAtLastError());
...
{
    softmax_kernel<<<cuda_gridsize(batch*groups), BLOCK, 0, get_darknet_stream()>>>(input, n, batch, batch_offset, groups, group_offset, stride, temp, output);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void upsample_gpu(float *in, int w, int h, int c, int batch, int stride, int forward, float scale, float *out)
{
    ...
    upsample_kernel<<<cuda_gridsize(size), BLOCK, 0, get_darknet_stream()>>>(size, in, w, h, c, batch, stride, forward, scale, out);
    check_error(cudaPeekAtLastError());
}
