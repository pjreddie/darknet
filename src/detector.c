...
void test_multiple_detector(char *datacfg, char *cfgfile, char *weightfile, char * filenames, float thresh, float hier_thresh)
{
    glob_t results;
    int ret = glob(filenames, 0, NULL, &results);
    if (ret != 0 ) {
    ...
    for (int i=0; i<results.gl_pathc; i++) {
        strncpy(input, results.gl_pathv[i], 256);
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "multiple")) {
        test_multiple_detector(datacfg, cfg, weights, filename, thresh, hier_thresh);
    }
