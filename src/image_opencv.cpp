    ...
    float precision, int draw_precision, char *accuracy_name, int dont_show, int mjpeg_port, double time_remaining)
{
    try {
        ...
        sprintf(char_buff, "current avg loss = %2.4f    iteration = %d    Time Remaining = %2.2f hours", avg_loss, current_batch, time_remaining);
        pt1.x = 15, pt1.y = draw_size + 18;
        pt2.x = pt1.x + 800, pt2.y = pt1.y + 20;
        cv::rectangle(img, pt1, pt2, CV_RGB(0, 255, 255), CV_FILLED, 8, 0);
        pt1.y += 15;
