...
void
delta_region_class(float *output, float *delta, int index, int nclass, int classes, tree *hier, float scale, int stride,
                   float *avg_cat, int tag) {
    int i, n;
    if (hier) {
        float pred = 1;
        while (nclass >= 0) {
            pred *= output[index + stride * nclass];
            int g = hier->group[nclass];
            int offset = hier->group_offset[g];
            for (i = 0; i < hier->group_size[g]; ++i) {
                delta[index + stride * (offset + i)] = scale * (0 - output[index + stride * (offset + i)]);
            }
            delta[index + stride * nclass] = scale * (1 - output[index + stride * nclass]);

            nclass = hier->parent[nclass];
        }
        *avg_cat += pred;
    } else {
        if (delta[index] && tag) {
        ...
        for (n = 0; n < classes; ++n) {
            delta[index + stride * n] = scale * (((n == nclass) ? 1 : 0) - output[index + stride * n]);
            if (n == nclass) *avg_cat += output[index + stride * n];
        }
    }
}
...
void forward_region_layer(const layer l, network net) {
    ...
    for (b = 0; b < l.batch; ++b) {
        if (l.softmax_tree) {
            ...
            for (t = 0; t < 30; ++t) {
                ...
                if (!truth.x) break;
                int nclass = net.truth[t * (l.coords + 1) + b * l.truths + l.coords];
                float maxp = 0;
                int maxi = 0;
                if (truth.x > 100000 && truth.y > 100000) {
                    for (n = 0; n < l.n * l.w * l.h; ++n) {
                        ...
                        float p = scale *
                                  get_hierarchy_probability(l.output + class_index, l.softmax_tree, nclass, l.w * l.h);
                        if (p > maxp) {
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, class_index, nclass, l.classes, l.softmax_tree, l.class_scale,
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
    ...
    printf("Region Avg IOU: %f, nclass: %f, Obj: %f, No Obj: %f, Avg Recall: %f,  count: %d\n", avg_iou / f_count,
           avg_cat / f_class_count, avg_obj / f_count, avg_anyobj / (l.w * l.h * l.n * l.batch), recall / f_count, count);
}
