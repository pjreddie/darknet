...
{
    binarize_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(x, n, binary);
...
{
    binarize_input_kernel<<<cuda_gridsize(size), BLOCK, 0, get_darknet_stream()>>>(input, n, size, binary);
    check_error(cudaPeekAtLastError());
}
...
{
    binarize_weights_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(weights, n, size, binary);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void smooth_layer(layer l, int size, float rate)
{
    ...
    smooth_kernel<<<cuda_gridsize(n), BLOCK, 0, get_darknet_stream()>>>(l.output_gpu, n, l.w, l.h, l.c, size, rate, l.delta_gpu);
    check_error(cudaPeekAtLastError());
}
