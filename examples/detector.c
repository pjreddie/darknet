...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)// ??ç†?èô?òô??ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô
{ // we must add validation data set training while train the training data set.
    ...
    // network??ç†?èô?òôÊπ≤Í≥ó?ìΩ ?ç†????ç†?èô?òô?ç†?? ngpus?ç†?? ?????ç†?èô?òô??ç†?èô?òô ??ç†?èô?òô ??ç†?èô?òô??ç†?èô?òô ??®Îì¶Ïª? ??ç†?èô?òô??ç†?èô?òô 
    ...
    */
    ...
    //load_args Ï¥àÍ∏∞?ôî ?ûë?óÖ
    list *plist = get_paths(train_images); // ?ï¥?ãπ ?ï®?àòÎ°? Í∞??†∏?ò§Î©? list?óê Í∞? nodeÎ°? ????û•?ù¥?ê®
    //int N = plist->size;
    char **paths = (char **)list_to_array(plist); // load_args?ùò pathsÍ∞? 2Ï∞®Ïõê ?è¨?ù∏?Ñ∞?ù¥Í∏? ?ïåÎ¨∏Ïóê list -> char**Î°? Î≥??ôò
	...
	printf("get_current_batch : %ld , net->max_batches : %d\n",get_current_batch(net),net->max_batches);
        if(l.random && count++%10 == 0){
            ...
            free_data(train);
            load_thread = load_data(args); // ngpusÍ∞? 1Í∞úÍ?? ?ïÑ?ãå Í≤ΩÏö∞?óê ?Ç¨?ö©?ê†  thread
        ...
        pthread_join(load_thread, 0); // waiting load_thread
        train = buffer;
        load_thread = load_data(args);// ?ã§?ùå Î∞òÎ≥µÎ¨∏Ïóê?Ñú ?Ç¨?ö©?ï† ?ù¥ÎØ∏Ï??Î•? Í∞??†∏?ò§?äî thread
...
#else
        loss = train_network(net, train); // yolo layerø°º≠ ±∏«— cost∏¶ ≈Î«œø© ∆Ú±’¿ª loss∑Œ ∆ƒæ«
#endif
        if (avg_loss < 0) avg_loss = loss; // loss∞ËªÍ¿Ã √≥¿Ω¿œ ∞ÊøÏ
        avg_loss = avg_loss*.9 + loss*.1; // ¿Ã¿¸±Ó¡ˆ ±∏«— loss¿« ≈©±‚∏¶ 0.9∑Œ «ˆ¿Á¿« loss¥¬ 0.1∑Œ ∞ËªÍ
...
void run_detector(int argc, char **argv) // argv[1] == detector ??ç†?èô?òô ÂØÉÏéå?ä¶ 
{
    ...
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);// ??ç†?èô?òô??ç†?èô?òô
