...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)// ?ÔøΩÔøΩ?ÔøΩÔøΩ ?ÔøΩÔøΩ?ÔøΩÔøΩ
{ // we must add validation data set training while train the training data set.
    ...
    // network?ÔøΩÔøΩÍ∏∞Ïùò Ôø???ÔøΩÔøΩÔø?? ngpusÔø?? ????ÔøΩÔøΩ?ÔøΩÔøΩ ?ÔøΩÔøΩ ?ÔøΩÔøΩ?ÔøΩÔøΩ Í≥µÍ∞Ñ ?ÔøΩÔøΩ?ÔøΩÔøΩ 
    ...
    */
    ...
    //load_args √ ±‚»≠ ¿€æ˜
    list *plist = get_paths(train_images); // «ÿ¥Á «‘ºˆ∑Œ ∞°¡Æø¿∏È listø° ∞¢ node∑Œ ¿˙¿Â¿Ãµ 
    //int N = plist->size;
    char **paths = (char **)list_to_array(plist); // load_args¿« paths∞° 2¬˜ø¯ ∆˜¿Œ≈Õ¿Ã±‚ ∂ßπÆø° list -> char**∑Œ ∫Ø»Ø
    ...
    *  val = (1-dy) * (1-dx) * get_pixel_extend(im, ix, iy, c) + 
        ...
        load_args a = Îß§Í∞úÎ≥??àò?ì§Í≥? Ï£ºÏÜåÎ•? Í≥µÏú†?ïòÍ∏? ?ïåÎ¨∏Ïóê Í≤∞Í≥ºÎ°? ?†Å?úºÎ°? Îß? Ï≤òÏùå?óê Ï§? Îß§Í∞úÎ≥??àò?ù∏ args?óê ????û•
    ...
    */
	...
	printf("get_current_batch : %ld , net->max_batches : %d\n",get_current_batch(net),net->max_batches);
        if(l.random && count++%10 == 0){
            ...
            free_data(train);
            load_thread = load_data(args); // ngpus∞° 1∞≥∞° æ∆¥— ∞ÊøÏø° ªÁøÎµ…  thread
        ...
        pthread_join(load_thread, 0); // waiting load_thread
        train = buffer;
        load_thread = load_data(args);// ¥Ÿ¿Ω π›∫ππÆø°º≠ ªÁøÎ«“ ¿ÃπÃ¡ˆ∏¶ ∞°¡Æø¿¥¬ thread
...
void run_detector(int argc, char **argv) // argv[1] == detector ?ÔøΩÔøΩ Í≤ΩÏö∞ 
{
    ...
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);// ?ÔøΩÔøΩ?ÔøΩÔøΩ
