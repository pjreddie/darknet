...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)// ?占쏙옙?占쏙옙 ?占쏙옙?占쏙옙
{ // we must add validation data set training while train the training data set.
    ...
    // network?占쏙옙湲곗쓽 占???占쏙옙占?? ngpus占?? ????占쏙옙?占쏙옙 ?占쏙옙 ?占쏙옙?占쏙옙 怨듦컙 ?占쏙옙?占쏙옙 
    ...
    */
    ...
    //load_args 초기화 작업
    list *plist = get_paths(train_images); // 해당 함수로 가져오면 list에 각 node로 저장이됨
    //int N = plist->size;
    char **paths = (char **)list_to_array(plist); // load_args의 paths가 2차원 포인터이기 때문에 list -> char**로 변환
    ...
    *  val = (1-dy) * (1-dx) * get_pixel_extend(im, ix, iy, c) + 
        ...
        load_args a = 留ㅺ컻蹂??닔?뱾怨? 二쇱냼瑜? 怨듭쑀?븯湲? ?븣臾몄뿉 寃곌낵濡? ?쟻?쑝濡? 留? 泥섏쓬?뿉 以? 留ㅺ컻蹂??닔?씤 args?뿉 ????옣
...
void run_detector(int argc, char **argv) // argv[1] == detector ?占쏙옙 寃쎌슦 
{
    ...
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);// ?占쏙옙?占쏙옙
